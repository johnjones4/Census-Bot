...
buildResponseFromWitResponse = (entities) => {
  ...
  if (entities.total_query) {
    if (entities.HETENURE && entities.HETENURE.length > 0 && entities.HETENURE[0].entities) {
      if (entities.HETENURE[0].entities.rent) {
        query.where('HETENURE',consts.inverseValues.HETENURE['OWNED OR BEING BOUGHT BY A HH MEMBER']);
        responseSummary.push('own a home');
      } else if (entities.HETENURE[0].entities.rent) {
        query.where('HETENURE',consts.inverseValues.HETENURE['RENTED FOR CASH']);
        responseSummary.push('rent a home');
      } else if (entities.HETENURE[0].entities.free) {
        // query.where('HETENURE',consts.inverseValues['RENTED FOR CASH']);
        responseSummary.push('live for free');
      }
    }
